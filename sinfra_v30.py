    ...
    def do_GET(self):
        ...
        if path == "/dashboard" or path == "/":
            ...
            if not user:
                ...
                tab_login_active = "active" if tab != "criar-conta" else ""
            ...
            if is_admin:
                ...
                for u_opt in todos_usuarios:
                    users_pass_select_opts += f'<option value="{u_opt["id"]}">{esc(u_opt["nome"])} ({u_opt["login"]}) - {esc(u_opt["email"])}</option>'
                ...
                <div class="section-card">
                  <h3 class="section-card-title">🔑 Alteração / Redefinição de Senha de Usuários (Exclusivo Administrador)</h3>
                  <p style="font-size: 12.5px; color: var(--text-secondary); margin-bottom: 14px;">Módulo exclusivo para o Administrador alterar a senha de qualquer usuário em caso de esquecimento.</p>
                  <form action="/redefinir_senha_admin" method="POST">
                    <div class="form-grid">
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <input type="password" name="nova_senha" class="form-input" placeholder="Digite a nova senha do usuário" required>
                      </div>
                    </div>
                    <button type="submit" class="btn-save-submit">🔑 Alterar Senha do Usuário</button>
                  </form>
                </div>
                '''
            else:
    ...
    def do_POST(self):
        ...
        if self.path == "/criar_conta":
            ...
            return
                ...
                # 8. ROTA ALTERAR SENHA DE USUÁRIO (EXCLUSIVO ADMINISTRADOR)
        if self.path == "/redefinir_senha_admin":
            user = self.get_cookie_user()
            if not user or user.get('perfil', '').lower() != 'admin':
                self.send_response(302)
                self.send_header('Location', '/dashboard?msg=' + urllib.parse.quote('Erro: Acesso restrito ao perfil de Administrador.') + '&mod=admin')
                self.end_headers()
                return

            # params is already parsed
            ...
            if user_id and nova_senha:
                ...
                conn = get_db()
                cursor = conn.cursor()
                cursor.execute("UPDATE usuarios SET senha_hash = ?, salt = ? WHERE id = ?", (senha_hash, salt, user_id))
                cursor.execute("SELECT login, nome FROM usuarios WHERE id = ?", (user_id,))
                u_target = cursor.fetchone()
                conn.commit()
                conn.close()

                u_label = u_target['login'] if u_target else f"ID {user_id}"
                self.send_response(302)
                self.send_header('Location', f'/dashboard?msg=' + urllib.parse.quote(f'Senha do usuário "{u_label}" alterada com sucesso pelo Administrador!') + f'&mod=admin')
                self.end_headers()
                return
            else:
                self.send_response(302)
                self.send_header('Location', '/dashboard?msg=' + urllib.parse.quote('Erro: Selecione o usuário e preencha a nova senha.') + '&mod=admin')
                self.end_headers()
                return
